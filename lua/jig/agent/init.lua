...
local function cmd_mcp_list()
  ...
  for _, file in ipairs(report.files) do
    local state = file.exists and "present" or "missing"
    local suffix = file.error ~= "" and (" error=" .. file.error) or ""
    table.insert(
      lines,
      string.format("- %s (%s, servers=%d)%s", file.name, state, file.server_count, suffix)
    )
  end
  ...
  for _, server in ipairs(report.servers) do
    ...
    table.insert(
      lines,
      string.format("- %s [%s] %s%s", server.name, status, server.command, suffix)
    )
  end
...
local function cmd_mcp_tools(opts)
  ...
  if result.ok and type(result.payload) == "table" then
    ...
    table.insert(lines, stringify(result.payload))
  end
